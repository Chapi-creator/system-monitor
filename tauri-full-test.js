...
async function main() {
  ...
  check('window.api con 16 métodos', apiShape === 'getAlwaysOnTop,getGpuInfo,getSessionStats,getSettings,getSystemStats,getTopProcesses,hideWidget,killProcess,onModeChanged,onThresholdsChanged,onVisibilityChanged,setThreshold,setThresholdDelta,setWidgetMode,showWidget,toggleAlwaysOnTop', apiShape);
  ...
  await sleep(1500);
  const stillRunning = ps(`powershell -NoProfile -Command "(Get-Process -Name 'System Monitor Widget-Tauri-Portable-1.1.1' -ErrorAction SilentlyContinue).Count"`);
  ...
  console.log('\n=== 9. Instancia única ===');
  const beforeInstances = ps(`powershell -NoProfile -Command "(Get-Process -Name 'System Monitor Widget-Tauri-Portable-1.1.1' -ErrorAction SilentlyContinue).Count"`);
  const second = spawn('"./dist/System Monitor Widget-Tauri-Portable-1.1.1.exe"', [], { shell: true, windowsHide: true, stdio: 'ignore' });
  await sleep(4000);
  const afterInstances = ps(`powershell -NoProfile -Command "(Get-Process -Name 'System Monitor Widget-Tauri-Portable-1.1.1' -ErrorAction SilentlyContinue).Count"`);
